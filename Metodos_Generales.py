...
def plot_xt(data, home_team, away_team):
	# Create a color gradient
	colors = np.linspace(0.2, 1, len(data))
	
	# Create a horizontal bar chart
	fig, ax = plt.subplots(figsize=(8, 6))
	fig.patch.set_facecolor('#111111')  # Set dark background color
	ax.set_facecolor("#111111")
	ax.edgecolor="white"
	# Plot the horizontal bars with reversed order
	bars=ax.barh(range(len(data)), sorted(data, reverse=False), color=plt.cm.Greens(colors[::1]))
	
	# Customize the chart
	ax.set_yticks(range(len(data)))
	ax.set_yticklabels(data.index[::-1], color="#ffffff") #Jugadores
	ax.set_xlabel('Xt', color="#111111")
	ax.set_ylabel('Player', color="#111111")
	fig.suptitle(f'{home_team} vs {away_team} Expected Threat', color='#ffffff', fontsize=16, y=.95)  # Set title color to white
	ax.xaxis.label.set_color('#ffffff')  # Set x-axis label color to white
	...
	for i, bar in enumerate(bars):
	    width = bar.get_width()
	    ax.text(width + .0005, bar.get_y() + bar.get_height() / 2, f'{width:.2f}', ha='left', va='center', color='white')
	
	# Adjust spacing
	plt.tight_layout()
	# Display the chart
	plt.show()
	plt.savefig(f'{home_team} vs {away_team} Expected Threat.jpg', format='jpeg')
	
def plot_green(data, title):
	# Create a color gradient
	colors = np.linspace(0.2, 1, len(data))
	...
	ax.set_xlabel('Xt', color="#111111")
	ax.set_ylabel('Player', color="#111111")
	fig.suptitle(f'{title}', color='#ffffff', fontsize=16, y=.95)  # Set title color to white
	ax.xaxis.label.set_color('#ffffff')  # Set x-axis label color to white
	ax.yaxis.label.set_color('#ffffff')  # Set y-axis label color to white
